...
IPベースのリクエスト制限:
- デフォルト: 60 req/min per IP
- `/auth/login`等の認証エンドポイントは厳格化（5 req/min）
...
- アルゴリズム: Fixed-window counter方式（[ADR-007](../adr/007-rate-limiting-strategy.md)）
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    """Fixed-window counter方式のRate Limiting Middleware。"""
    
    SKIP_PATHS = frozenset({"/auth/health", "/docs", "/openapi.json"})
...
> `registrationAllowed` は `false`。ユーザー登録は管理者（tenant_admin以上）がAdmin API経由で行う運用。
> `passwordPolicy` は現時点で未設定。将来的にテナント単位でポリシーを設定可能にする想定。

### Required Actions

| alias | 用途 | defaultAction |
|-------|------|---------------|
| `VERIFY_EMAIL` | メール確認 | false |
| `CONFIGURE_TOTP` | MFA設定（任意） | false |
| `UPDATE_PASSWORD` | パスワード更新 | false |
...
> **現在の実装**: `registrationAllowed: false` のため、ユーザー登録は管理者（tenant_admin以上）がAdmin API経由で行う。
> 将来的にセルフ登録を有効化する場合は、`registrationAllowed: true` + `verifyEmail: true` に変更する。
...
| [ADR-007](../adr/007-rate-limiting-strategy.md) | Fixed-window counter方式 |
